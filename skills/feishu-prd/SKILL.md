...
**文档内引用挂内链**——正文提到本文档内的屏 / 章节时写 `[[ref:anchorId|显示文本]]`（anchorId = 目标 section 的 `anchorId` 字段），交付层自动转飞书锚点链接。跨章引用用「章名 / 节名」+ 内链，不写「第 N 章」硬编号（编号是交付层生成的，写作期引用必错位）。页面跳转描述一律挂内链，这是长期写作习惯；机制细节见 `delivery/blocks-cheatsheet.md`「内链」节。**`[[ref:]]` 一律写在 `**…**` 粗体标记之外**——ref 嵌进粗体内部时，导入器会把 `**` 按字面星号写进文档（静默劣化），ref pass 也无法定位该块；`validateManifest` 已 fail-hard 拦截。要强调的词收进纯文本部分加粗，链接本身的样式已足够醒目。

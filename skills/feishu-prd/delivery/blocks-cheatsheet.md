...
- ref 可与 `**粗体**`、行内代码（成对反引号）**同块共存**；但 **ref 嵌进 `**…**` 内部 = 禁止**——导入器对「粗体包 ref」不转样式，把 `**` 按字面星号写进文档（静默劣化），ref pass 定位失配 FATAL；`validateManifest` 已在交付前 fail-hard 拦截，把粗体收到纯文本部分即可。与其它行内标记混用会直接 FAIL 拒绝重建——先扩展解析器再把这种组合写进 manifest。
